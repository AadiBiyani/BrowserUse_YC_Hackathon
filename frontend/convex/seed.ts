...
export const seedAll = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    const experimentId = await ctx.db.insert("experiments", {
      name: JOB_ID,
      taskGoal:
        "Fill in the form: set customer name to 'ReplayBench Test', telephone to '555-1234', email to 'test@replaybench.dev', select 'Medium' for pizza size, then submit.",
      taskUrl: "https://httpbin.org/forms/post",
      successConditions: [
        "Response body contains 'ReplayBench Test'",
